...
**Deploy button:** click the "Deploy on Railway" badge in the README or use the link:

```
https://railway.com/template/from?repoUrl=https://github.com/openbotx/openbotx
```

### How it works

1. Railway reads `railway.json` from the repository
2. It builds the Docker image using the `Dockerfile`
3. The service starts and Railway assigns a public URL
...
**Deploy button:** click the "Deploy to Render" badge in the README or use the link:
...
2. It builds the Docker image using the `Dockerfile`
3. The service starts on the `free` plan with a public URL
4. Render sets the `PORT` environment variable to `8000`
...
**Deploy button:** click the "Deploy to Heroku" badge in the README or use the link:

```
https://heroku.com/deploy?template=https://github.com/openbotx/openbotx
```

### How it works

1. Heroku reads `app.json` for app metadata and detects `"stack": "container"`
2. It reads `heroku.yml` to know how to build the Docker image
3. The Docker image is built using the `Dockerfile`
4. The service starts and Heroku assigns a public URL
5. Heroku dynamically sets the `PORT` environment variable
...
**Deploy button:** click the "Deploy to DigitalOcean" badge in the README or use the link:
...
1. DigitalOcean reads `.do/app.yaml` from the repository
2. It builds the Docker image using the `Dockerfile`
3. The service starts on a `basic-xxs` instance with a public URL
4. DigitalOcean routes HTTP traffic to port `8000`
...
|---|---|---|
| `name` | `openbotx` | Application name |
| `services[0].name` | `web` | Service name |
| `services[0].dockerfile_path` | `Dockerfile` | Path to the Dockerfile |
| `services[0].http_port` | `8000` | Port the app listens on |
| `services[0].instance_count` | `1` | Number of instances |
| `services[0].instance_size_slug` | `basic-xxs` | Smallest instance size |
...
| `.do/app.yaml` | DigitalOcean | App Platform service definition |
