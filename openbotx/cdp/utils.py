...
class LoggerMixin:
    def __init__(self, *args, **kwargs):
        ...
        self._logger = logging.getLogger(f"cdp.{type(self).__name__}")


class ContextLoggerMixin(LoggerMixin):
    logging.getLogger("cdp.ContextLoggerMixin")

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self._logger = logging.LoggerAdapter(
            logging.getLogger(f"cdp.ContextLoggerMixin.{type(self).__name__}"),
            {},
        )
        self.set_logger_context(realname=f"cdp.{type(self).__name__}")
...
class Retry(LoggerMixin):
    def __init__(
        ...
        *,
        retries: int = 1,
        on_error: str | t.Callable[[], t.Awaitable[None]] | None = None,
        ...
        for i in range(self._retries + 1):
            try:
                ...
                if i == self._retries:
                    raise e
                if self._log_errors:
                    frame = self._get_appcode_frame(sys.exc_info()[-1])
...
def retry_on(
    *exception_class: type[BaseException],
    loop: IEventLoop,
    retries: int = 1,
    delay: float | tuple[float, float] = 0.0,
...
class SubtaskSpawner(Closable):
    """Keeps track of spawned async tasks."""
    ...
    async def wait_subtasks(self):
        """Wait all current subtasks to finish and return their results."""
        return await asyncio.gather(
            *(asyncio.shield(task) for task in self._subtasks), return_exceptions=True
        )
    ...
    def _create_subtask(
        self,
        coro: t.Coroutine[t.Any, t.Any, _T] | asyncio.Future[_T],
        name=None,
    ) -> asyncio.Future[_T]:
        if not self.is_open:
        ...
        try:
            ...
            if self._exception_handlers > 0:
                if not self._exception_waiter.done():
                    self._logger.debug(
                        "firing the exception handler for subtask %s",
                        repr(task),
                    )
                    self._exception_waiter.set_exception(task.exception())
                else:
                    self._logger.exception(
                        "error in subtask %s but exception handler was already fired:",
                        repr(task),
                    )
            else:
                self._logger.exception("error in subtask %s:", repr(task))


class Worker(SubtaskSpawner, WorkerBase):
    """Daemon object that does some kind of work."""

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self._subworkers: list[Worker] = []
