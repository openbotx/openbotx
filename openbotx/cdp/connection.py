...
from aiohttp.http_websocket import WSCloseCode, WSMsgType

from openbotx.cdp import protocol as cdp
from openbotx.cdp.base import IEventLoop
from openbotx.cdp.exceptions import (
    CDPBrowserError,
    CDPConnectionClosed,
    CDPEventListenerClosed,
    CDPInternalError,
    CDPSessionClosed,
)
from openbotx.cdp.utils import ContextLoggerMixin, LoggerMixin, SingleTaskWorker, retry_on
...
class CDPEventListener:
    ...
    def __str__(self) -> str:
        return (
            ...
            f"(buffer={self._queue.qsize()}/{self._queue.maxsize}, closed={self._closed})"
        )
...
class CDPBase(LoggerMixin):
    ...
    def __init__(
        ...
        super().__init__()
        self._listeners: dict[type, set[CDPEventListener]] = defaultdict(set)
        self._id_iter = itertools.count()
        self._inflight_cmd: dict[int, tuple[t.Generator[dict, dict, t.Any], asyncio.Future]] = {}
    ...
    def listen(self, *event_types: type[T], buffer_size=100) -> t.AsyncIterator[T]:
        """Return an async iterator that iterates over events matching the indicated types."""
    ...
    async def wait_for(self, event_type: type[T]) -> t.AsyncGenerator[T, None]:
        ...
        with block. The block will not exit until the indicated event is received.
        """
    ...
    def safe_wait_for(self, event_type: type[T]) -> t.Generator[t.Awaitable[T], None]:
        ...
        This context manager yields an awaitable that should be awaited to receive the event.
        Use this to register an event listener before performing the action which will
        trigger the event (like a page navigation), avoiding race conditions of wait_for().
        """
    ...
    async def _async_wait_for(self, event_type: type[T]) -> T:
        async for event in self.listen(event_type, buffer_size=2):
            return event
    ...
    def _handle_cmd_response(self, data):
        ...
        This will set an event flag that will return control to the task
        ...
        if "error" in data:
            ...
            try:
                ...
                event.set_exception(
                    CDPInternalError("the command's generator function did not exit when expected!")
                )
        ...
        for listener in self._listeners[type(event)]:
            try:
                ...
                self._logger.warning(
                    "event %s dropped because listener %s queue is full",
                    type(event),
                    listener,
                )
...
class CDPConnection(CDPBase, SingleTaskWorker):
    ...
    You should generally call the :func:`connect_cdp()` instead of
    ...
    async def connect(self):
        ...
        self._ws = await self._http_client.ws_connect(
            self._wsurl, compress=15, autoping=True, autoclose=True
        ).__aenter__()
            ...
            if message.type == WSMsgType.TEXT:
                ...
                if "sessionId" in data:
                    ...
                    try:
                        ...
                        self._logger.debug("received message for unknown session: %s", data)
                        continue
...
class CDPSession(CDPBase, ContextLoggerMixin):
    ...
    async def dom_enable(self):
        """Context manager that executes dom.enable() on enter and dom.disable() on exit.

        Keeps track of concurrent callers and only disables DOM events when
    ...
    async def page_enable(self):
        """Context manager that executes page.enable() on enter and page.disable() on exit.

        Keeps track of concurrent callers and only disables page events
...
async def connect_cdp(url: str) -> CDPConnection:
    """Connect to the browser specified by the debugging URL.

    This connection is not automatically closed! You can either use the connection
    object as a context manager (``async with conn:``) or else call
    ``await conn.close()`` on it when you are done with it.
    """
    http = ClientSession()
